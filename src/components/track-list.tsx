...
import { ListMusic, Music, Upload } from "lucide-react";
...
export function TrackList() {
  ...
  const { tracks, loading } = useTracks(); // Removido addTrack

  const [selectedCategory, setSelectedCategory] = useState<string | null>(null);
  // Removido isUploadOpen

  const isAdmin = role === 'admin';

  // Removido handleAddNewTrack
  ...
  const renderContent = () => {
            ...
            <Card key={categoryName} className="flex flex-col transition-all hover:shadow-lg">
              <CardHeader className="flex-row items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{tracksByCategory[categoryName].length} faixas</p>
                </div>
              </CardHeader>
              <CardContent className="flex-grow"></CardContent>
              <CardFooter className="justify-center">
    ...
    return (
       <div className="text-center py-16 border-2 border-dashed rounded-lg">
          ...
          {isAdmin && <p className="text-muted-foreground mt-2">Clique em "Enviar Faixa" para criar uma nova faixa e categoria.</p>}
        </div>
    );
  };
  ...
  return (
    <div className="container py-12">
      <div className="flex flex-col sm:flex-row items-center sm:justify-between gap-4 mb-8 text-center sm:text-left">
        <h2 className="text-xl font-bold">Suas Categorias de Músicas</h2>
        {/* Removido o botão Enviar Faixa */}
      </div>
